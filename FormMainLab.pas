...
// Проверяет, сохранен ли файл, чтобы продолжить действие
...
var
  ...
  count: integer;
begin
  wrd := InputBox('Поиск слов по множеству символов',
                    'Введите последовательность символов, с которой могут начинаться слова/о', '');
  if wrd <> '' then
    begin
      count:= FTree.FindWordsWithStr(wrd);
      Str(count, s);
      MemoResult.Lines.Add('Слов начинающихся с "'+ wrd +'" : ' + s);
    end
  else
